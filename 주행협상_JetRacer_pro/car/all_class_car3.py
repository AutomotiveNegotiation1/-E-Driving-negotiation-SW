...
# Finally, execute the cell below to make the racecar move forward, 
# steering the racecar based on the x value of the apex.
# 
...
handler = RotatingFileHandler("log_all_class.log", mode='w',
...
log.info("Car IP address: %s",(([ip for ip in socket.gethostbyname_ex(socket.gethostname())[2] 
                                ...
                                s.getsockname()[0], s.close()) for s 
                                in [socket.socket(socket.AF_INET, socket.SOCK_DGRAM)]][0][1]]) + ["no IP found"])[0])
...
class JoystickController:
    ...
    def __init__(self,car,controller):   
        ...
        left_link = traitlets.dlink((self.controller.axes[0], 'value'), 
                                    (self.car, 'steering'), transform=lambda x: -x)
        right_link = traitlets.dlink((self.controller.axes[5], 'value'), 
                                     (self.car, 'throttle'), transform=lambda x: -x)
...
class Location:
    ...
    def __init__(self):
        ...
        self.center_junction_location = Point(42,258)
...
class Notify(threading.Thread):
    ...
    def __init__(self,car):
        ...
        # For class B & C
        self.other_car_asked_over_taking = 0
        self.car_over_taking_marker_id = 0
        self.enable_over_taking_flag = 0
        self.other_car_over_taking = 0
            ...
            if data["Car_id"] != CAR_ID:
                ...
                if self.default_class == CLASS_A:
                    if (data["road_type"] == ROAD_RIGHT_FLOW) and (data["cmd"] == ASK_FOR_RIGHT_FLOW):
                        print('Car {} asked for right flow'.format(data["Car_id"]))
                        log.info('Get PIM_Req payload: %s',payload)
                        msg_log = 'Get PIM_Req payload: {}'.format(payload)
                        self.send_logging(msg_log)
                        #self.car_status = "PIM RX"
                    if data["cmd"] == RES_OK_FOR_PIM:
                        log.info('Get PIM_Done payload: %s',payload)
                        msg_log = 'Get PIM_Done payload: {}'.format(payload)
                        self.send_logging(msg_log)
                ...
                if self.default_class == CLASS_B:
                    ...
                    if (data["cmd"] == DMM_BROADCAST):
                        ...
                        self.send_logging(msg_log) 
                        self.other_car_asked_over_taking = 1
                        self.car_over_taking_marker_id = int(data["marker_id"])
                        self.car.throttle = 0.148
                ...
                if self.default_class == CLASS_C:
                    if (data["cmd"] == ASK_FOR_OVER_TAKING):
                        log.info('Get DNM_Req payload %s',payload)
                        msg_log = 'Get DNM_Req payload: {}'.format(payload)
                        self.send_logging(msg_log) 
                        log.info('Get DNM_Req: Sender %s, Receiver %s, Remain Distance %s',
                                 data["marker_id"],data["receiver_marker_id"],data["remain_distance"])
                        msg_log = ('Get DNM_Req: Sender {}, Receiver {}, '
                        'Remain Distance {}'.format(data["marker_id"],data["receiver_marker_id"],
                                                    data["remain_distance"]))
                        self.send_logging(msg_log) 
                        self.other_car_asked_over_taking = 1
                        self.car_over_taking_marker_id = int(data["marker_id"])
                        self.car.throttle = 0
                        log.info('Stop car when get DMM request')
                        self.car_status = "DNM RES"
                    if (data["cmd"] == DONE_OVER_TAKING):
                        log.info('Get DNM_Done payload %s',payload)
                        msg_log = 'Get DNM_Done payload: {}'.format(payload)
                        self.send_logging(msg_log) 
                        log.info('Get DNM_Done: Sender %s, Receiver %s, '
                                 'NegoDrivingDone:Done',data["marker_id"],data["receiver_marker_id"])
                        msg_log = ('Get DNM_Done: Sender {}, Receiver {}, '
                        'NegoDrivingDone:Done'.format(data["marker_id"],data["receiver_marker_id"]))
                        self.send_logging(msg_log) 
                        self.other_car_over_taking = 0
            ...
            if data["Car_id"] == CAR_ID:
                ...
                if (data["Command"] == "Scenario"):
                    if (data["Class"] == "A"):
                        ...
                        self.location.entry_junction_location = Point(150, 207)
                        self.location.exit_junction_location = Point(55,430)
            ...
            if self.default_class == CLASS_A:
                ...
                if self.enable_moving == 1:
                    ...
                    if self.car.throttle > 0:
                        self.car.throttle = 0
            if self.default_class == CLASS_B:
                if self.other_car_asked_over_taking == 1:
                    self.other_car_asked_over_taking = 0 
                    self.other_car_over_taking = 1
                    jsondata = {
                        "Car_id": CAR_ID,
                        "marker_id":CAR_MARKER_ID,
                        "marker_target": self.car_over_taking_marker_id,
                        "cmd": RES_OK_FOR_DMM
                    } 
                    log.info("DMM Response payload%s",jsondata)
                    msg_log = 'Send DMM Response payload: {}'.format(jsondata)
                    self.send_logging(msg_log)
                    log.info("DMM Response: Sender %s, Receiver %d, AgreementFlag:1",
                             CAR_MARKER_ID,self.car_over_taking_marker_id)
                    msg_log = ('Send DMM Response: Sender {}, Receiver {}, '
                    'AgreementFlag:1'.format(CAR_MARKER_ID,self.car_over_taking_marker_id))
                    self.send_logging(msg_log)
                ...
                self.car.steering = steering - 0.21
                ...
                if self.enable_moving == 1:
                    if len(close_stopped) > 0:
                        ...
                        if (self.car.throttle == 0) and self.mode == AUTO_MODE:                        
                            if self.other_car_over_taking == 0:
                                self.resumeCar()
                                print('Resume car')
                else:
                    if self.car.throttle > 0:
            ...
            if self.default_class == CLASS_C:
                if self.other_car_asked_over_taking == 1:
                    ...
                    jsondata = {
                        ...
                        "marker_id":CAR_MARKER_ID,
                        "marker_target": self.car_over_taking_marker_id,
                        "cmd": RES_OK_FOR_OVER_TAKING
                    } 
                    log.info("Send response DNM :%s",jsondata)
                    msg_log = 'Send response DNM payload: {}'.format(jsondata)
                    self.send_logging(msg_log)
                    log.info("DNM Response: Sender %s, Receiver %d, AgreementFlag:1",
                             CAR_MARKER_ID,self.car_over_taking_marker_id)
                    msg_log = ('DNM Response: Sender {}, Receiver {},'
                    'AgreementFlag:1'.format(CAR_MARKER_ID,self.car_over_taking_marker_id))
                    self.send_logging(msg_log)
                    self.send_road_info(json.dumps(jsondata))
                    # Stop car
